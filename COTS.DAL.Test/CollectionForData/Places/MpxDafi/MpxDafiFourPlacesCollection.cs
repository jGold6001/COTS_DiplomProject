    ...
    public static class MpxDafiFourPlacesCollection
    {
        public static List<Place> Get()
        {
            ...
            for (int s = 4; s > 0; s--)
            {
                long id = startIndex++;
                int row = 1;
                var place = new Place(id, row, s, MpxDafiHallsCollection.Get()[3].Id, MpxSectorsCollections.Get()[0].Id);
                places.Add(place);
            }
            for (int s = 6; s > 0; s--)
            {
            ...
            for (int s = 12; s > 0; s--)
            {
                long id = startIndex++;
                int row = 8;
                var place = new Place(id, row, s, MpxDafiHallsCollection.Get()[3].Id, MpxSectorsCollections.Get()[1].Id);
                places.Add(place);
            }
